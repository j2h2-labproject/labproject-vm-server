    ...
    start: function(config, logger, onstart, onstop, callback) {
        ...
        if (hypervisor_string.is_hypervisor(config.hypervisor)) {
            ...
            hypervisor_util.is_libvirt_running(function(error, result) {
                ...
                // Start the transport for communications with the frontend server
                ...
                if (transport == null) {
                    ...
                    config.server_name, // Set server name
                    function(error, address, port, emitter) { // Set function to run on transport start


                        var status_handler = require(LABPROJECT_LIB + "/handlers/status_handler");
                        status_handler.set_logger(logger);

                        status_handler.start_pulse(function(error, data){
                            emitter("server_pulse", data);
                        });

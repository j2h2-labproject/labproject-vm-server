...
module.exports = {
    start_transport: function (logger, server_name, on_start, on_data, on_end, config) {
        ...
        try {
            ...
            function server_send(out_id, data) {
                server.emit(out_id + "." + server_name, data);
            };

            on_start(null, "0.0.0.0", config.port, server_send);
        } catch (error) {
            console.log(error);

...
module.exports = {
	...
	handles: function(command) {
		valid_commands = ['switch_exists', 'switch_status', 'create_switch', 'delete_switch', 'list_ports', 'get_port_data', 'connect_port', 'disconnect_port', 'set_port_mode', 'clear_port', 'set_trunks', 'patch_ports']
...
function connect_port(switch_id, ports, callback) {
	if (switch_id === undefined) {
		...
		list_ports(switch_id, function(l_error, port_list) {
			foreach(ports,
				function(loc, port, pass_data, next) {

					// Check if the port is already attached
					if (port_list.indexOf(port) == -1) {
			...
			function(loc, port, pass_data, next) {
				command.run('sudo',["ovs-vsctl", "set", "interface", port, "type=patch"], function(error, stdout, stderr){
					if (!error)
					{
						...
						next("Could not set ports to patch ports: " + error.message, null);
					}
				});
			},

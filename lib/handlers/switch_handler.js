	...
	handle: function(command, params, callback) {
		switch (command) {
			...
			case "list_ports":
				...
				break;
			case "connect_port":
				connect_port(params['switch_id'], params['port'], callback);
				break;
			case "disconnect_port":
				disconnect_port(params['switch_id'], params['port'], callback);
				break;
			case "get_port_data":
			...
			if (!error)
				{
					...
					callback("Could not create switch. An error occured", null);
				}
		});
	}
}
...
function connect_port(switch_id, port, callback) {
	if (switch_id === undefined) {
		callback("No switch ID was set", null);
	} else if (port === undefined) {
		callback("No port was set", null);
	} else {
...
function disconnect_port(switch_id, port, callback) {
	if (switch_id === undefined) {
		callback("No switch ID was set", null);
	} else if (port === undefined) {
		callback("No port was set", null);

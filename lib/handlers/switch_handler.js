...
logger = new logging.logger("SWITCH", "cli");
...
function connect_port(switch_id, ports, callback) {
	if (switch_id === undefined) {
		...
		setup_ports(switch_id, 0, ports, function(error, status) {
			callback(error, status);
		});
	}
}

function setup_ports(switch_id, location, port_list, callback) {
	if (location >= port_list.length) {
		callback(null, true);
	} else {
		var port = port_list[location];
		command.run('sudo',["ovs-vsctl","add-port", switch_id, port, "tag=1", "vlan_mode=access"], function(error, stdout, stderr){
				if (!error && stderr[0] == '')
				{
					setup_ports(switch_id, location+1, port_list, callback)
				}else{
					disconnect_port(switch_id, [port], function(error, status) {
						callback("Could not add port " + port + " to switch " + switch_id + ". An error occured", null);
					});
				}
		});
	}
...
function disconnect_port(switch_id, ports, callback) {
	if (switch_id === undefined) {
		...
		remove_ports(switch_id, 0, ports, callback);
	}
}

function remove_ports(switch_id, location, port_list, callback) {
	if (location >= port_list.length) {
		callback(null, true);
	} else {
		var port = port_list[location];
		command.run('sudo',["ovs-vsctl","del-port", switch_id, port], function(error, stdout, stderr){
				if (!error)
				{
					remove_ports(switch_id, location, port_list, callback);
				}else{
					logger.log('error', error);
					callback("Could not remove port " + port + " from switch " + switch_id + ". An error occured", null);
				}
		});
	}
}

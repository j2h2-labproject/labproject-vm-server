...
module.exports = {
        ...
        try {
            switch (command) {
                ...
                case "deallocate_interface_group":
                    ...
                    break;
                case "allocate_maint_interface":
                    allocate_maint_interface(callback);
                    break;
                default:
    ...
    handles: function(command) {
        valid_commands = [
            ...
            "allocate_interface_group",
            "deallocate_interface_group", 
            "interface_exists",
            "allocate_maint_interface"
        ];
...
function allocate_maint_interface(callback) {

    var interface_num = Math.floor(Math.random() * 0xFFFFFFF).toString(16).toUpperCase();

    var interface_name = "mai" + interface_num;
    interface_util.create_tap_interface(interface_name, function(int_error, status){
        if (int_error) {
            callback(int_error, null);
            return;
        }

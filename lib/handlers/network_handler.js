...
module.exports = {
		...
		try {
			switch (command) {
				...
				case "interface_exists":
					interface_exists(params['devicename'], callback);
					break;
	...
	handles: function(command) {
		valid_commands = ['create_tap_device', "remove_tap_device", "create_external_interface", "allocate_interface_group",
...
function deallocate_interface_group(group_num, callback) {
    ...
    if (group_num > 99999) {
      ...
      remove_tap_device("lpif" + group_num, function(error, result) {
        if (!error) {
          callback(null, true);
        } else {
          callback("Could not allocate group", null);

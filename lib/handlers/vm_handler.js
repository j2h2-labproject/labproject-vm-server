...
module.exports = {
      ...
      return;
		...
		driver.configure(config.driver_config, function(error, status) {
			switch (command) {
				...
				// 		if (is_valid_config(params['config'])) {
				// 			var vm_config = insert_variables(params['config']);
				// 			driver.update_vm(vm_config, callback);
				// 		} else {
				// 			callback("Invalid config", null);
				// 		}
				// 		break;
	...
	handles: function(command) {
		valid_commands = ['vm_exists', 'create_vm', 'delete_vm', 'list_snapshots', 'start_vm', 'stop_vm', 'vm_is_running', 'suspend_vm', 'update_vm_config', 'get_vm_config', 'restore_vm_snapshot', 'make_vm_snapshot', 'delete_vm_snapshot', 'vm_console', 'attach_vm_drive', 'detach_vm_drive']

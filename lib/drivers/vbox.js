    ...
    VBoxManage(['createvm', '--name', name, '--uuid', uuid, '--register'], function(error, stdout, stderr){
        ...
        VBoxModify(uuid, config_array, function(error, stdout, stderr){
            ...
            setup_drives(uuid, vm_config.hd_list, vm_config.cdrom_list, function(d_error, result) {
              if (!d_error) {
                add_interfaces(uuid, vm_config.interface_list, function(i_error, result) {
                  if (!i_error) {
                    callback(null, true);
                  } else {
      ...
      if (!error && result === true) {
        ...
        function update_lists() {
          update_drives(uuid, vm_config.hd_list, vm_config.cdrom_list, function(d_error, result) {
            if (!d_error) {
              update_interfaces(uuid, vm_config.interface_list, function(i_error, result) {
                if (!i_error) {
                  callback(null, true);
                } else {
                  callback(i_error, null);
...
function attach_cds(uuid, cd_list, callback) {
  foreach(cd_list,
    ...
    function(loc, interface_item, pass_data, next) {
      interfaces.create_tap_interface(interface_item, function(c_error, result) {
        if (!c_error) {
          ...
          next("Could not create interface " + interface_list[loc], null);
        }
      });
    },
    callback);
    ...
    function(loc, interface_name, pass_data, next) {
      interfaces.remove_tap_interface(interface_name, function(d_error, result) {
        if (!d_error) {
          ...
          next("Could not remove interface " + interface_list[loc], null);
        }
      });
    },
    callback);
...
function delete_host_interfaces(uuid, callback) {
  get_vminfo(uuid, function(error, config_data) {
    ...
    delete_interfaces(interface_list, function(d_error, result) {
...
function update_hd(uuid, drive_updates, callback) {
  ...
  if (drive_updates === undefined || drive_updates === null) {
    callback(null, true);
    return;
  }
...
function update_interfaces(uuid, interface_updates, callback) {
  ...
  if (interface_updates === undefined || interface_updates === null) {
    callback(null, true);
    return;
  }
  ...
  foreach(interface_updates,
    function(loc, iface_config, pass_data, next) {
      ...
      // Remove the interface
        ...
        config_array = [
          '--nic' + (loc + 1), 'bridged',
          '--bridgeadapter' + (loc + 1), iface_config.interface
        ];

        interfaces.create_tap_interface(iface_config.interface, function(c_error, result) {
          if (!c_error||c_error.message == "Device already exists") {
            ...
            next("Could not create interface " + iface_config.interface, null);
          }
        });
      }
    },
